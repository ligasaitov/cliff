...
// MAIN SLIDER
var $status = $('.main__slider_counter');
var $slickElement = $('.main__slider_wrapper');
const spanS = '<span>';
const spanE = '</span>';

$slickElement.on('init reInit afterChange', function (event, slick, currentSlide, nextSlide) {
	var i = (currentSlide ? currentSlide : 0) + 1;
	$status.html(i + ' / ' + spanS + slick.slideCount + spanE);
});

if($('.main__slider_wrapper').length>0){
	$('.main__slider_wrapper').slick({
		...
		//asNavFor:'',
		//appendDots:
		appendArrows:$('.main__slider_wrapper_arrows'),
...
if($('.models__slider').length>0){
	$('.models__slider').slick({
		...
		//asNavFor:'',
		//appendDots:
		appendArrows:$('.models__slider_arrows'),
		nextArrow:'<button type="button" class="slick-next"></button>',
		prevArrow:'<button type="button" class="slick-prev"></button>',
		responsive: [{
			breakpoint: 1253,
...
function init() {
    ...
    var myMap = new ymaps.Map("map", {
        ...
        controls: [],
        center: [56.7599972909026,60.80219850456276],
        // Уровень масштабирования. Допустимые значения:
        // от 0 (весь мир) до 19.
        zoom: 17
    });

    myPlacemark = new ymaps.Placemark([56.7599972909026,60.80219850456276],{
        id:'1'
    },
    {
    // Опции.
